...
class BatchEnv(gym.Env):
    ...
    def __init__(self, seed: int = None):
        ...
        self.observation_space = spaces.Box(low=np.array([0, 0, 0, 0, 20, 2.5, 1.4, 0.0], dtype=np.float32),
                                            high=np.array([10, 10, 10, 10, 100, 17.5, 6.6, 1.0], dtype=np.float32),
                                            shape=(8,))
    ...
    def _get_obs(self):
        base_state = self.simulator._get_obs()  # shape (7,)
        t_norm = self.step_count / self.max_steps  # scalar in [0,1)
        return np.concatenate([base_state, [t_norm]])  # shape (8,)
    ...
    def reset(self, seed: int = None, options: dict = None):
        ...
        return self._get_obs(), {'time': self.simulator.time}
    ...
    def step(self, action: int):
        ...
        # 2. HANDLE FEED ACTIONS (non-STOP)
            ...
            phi_prev = prev_state[6] * prev_state[2] * prev_state[
                4]  # - 0.5 * self.cum_A_fed - prev_state[ 5] * self.cum_B_fed
            phi_next = next_state[6] * next_state[2] * next_state[
                4]  # - 0.5 * self.cum_A_fed - next_state[5] * self.cum_B_fed
            shaping = 0.01 * (self.gamma * phi_next - phi_prev)
            reward += shaping
            # time penalty for continuing
            reward += -5.0
...
class Simulator:
    ...
    def step(self, clean: bool, F_A: float, F_B: float):
        ...
        info = {'time': self.time, 'sold_D': 0.0, 'used_A': F_A * self.dt * self.C_A_0,
                'used_B': F_B * self.dt * self.C_B_0, 'overflow': overflow}
...
if __name__ == "__main__":
    ...
    print(
        f"A: {state[0]:.2f} mol/L, B: {state[1]:.2f} mol/L, D: {state[2]:.2f} mol/L, U: {state[3]:.2f} mol/L, V: {state[4]:.2f} L, P_B: {state[5]:.2f} $/mol, P_D: {state[6]:.2f} $/mol")
    reward_list = []
    for a in action_list:
        ...
        print(f"Time: {info['time']} hr")
        print(
            f"A: {next_state[0]:.2f} mol/L, B: {next_state[1]:.2f} mol/L, D: {next_state[2]:.2f} mol/L, U: {next_state[3]:.2f} mol/L, V: {next_state[4]:.2f} L, P_B: {next_state[5]:.2f} $/L, P_D: {next_state[6]:.2f} $/mol")
